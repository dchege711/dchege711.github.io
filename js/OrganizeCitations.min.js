...
const citationIconClass=citationElement.parentElement.getAttribute("citation-icon-class");const citedByCount=citationElement.parentElement.getAttribute("cited-by-count")
citationIDToDetails[citationElement.id]={hoverText:citationElement.parentElement.innerText,iconHTML:(citationIconClass?`<i class="${citationIconClass}" style="margin-left:3px; margin-right:3px;" aria-hidden="true"></i>`:""),citationRefIds:[],citedByCountHTML:(citedByCount?`<sup>${citedByCount}</sup>`:"")};}
...
for(let i=0;i<citationElements.length;i++){const citationElement=citationElements[i];for(let citationRefId of citationIDToDetails[citationElement.id].citationRefIds){let citationRefElem=document.createElement("a");citationRefElem.href=`#${citationRefId}`;citationRefElem.innerText="^";citationRefElem.title=`Jump up to ${citationRefId}`;citationRefElem.classList.add("citation-ref-backlink");citationElement.parentElement.insertAdjacentElement("beforeend",citationRefElem);}}}
